...
int main(int arc, char** argv)
{
  ...
  /* Do a semaphore P-operation (Decrement/Wait) */
  printf("Program semb (PID: %d) about to do a P-operation.\n", getpid());
  ...
  if (semop(id, operations, 1) == 0) {
    /*
     * BLOCKS here waiting for a V by sema 
     */
    printf("Successful P-operation by program semb (PID: %d).\n", getpid());
  } else {
